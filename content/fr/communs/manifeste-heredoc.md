...
**Repository (Dépôt)**
L'espace numérique où vit un projet logiciel : une bibliothèque contenant tous les fichiers, l'historique des modifications, et les règles du projet.

**Maintainers**
Les gardiens du projet original, qui décident quelles contributions accepter ou rejeter (représentant l'autorité centrale contestée par le manifeste).

**Fork**
L'acte de copier entièrement un projet pour créer sa propre version indépendante. Le fork maintient un lien généalogique avec l'original tout en permettant une évolution autonome (une "bouture" de plante).

**Pull Request**
Une proposition de modification : "Voici les améliorations que j'ai apportées dans mon *fork*, voulez-vous les intégrer dans votre version ?"

**Rhizome**
Concept philosophique (Deleuze et Guattari) matérialisé par l'architecture technique : une structure sans centre ni hiérarchie fixe, où le savoir circule entre pairs égaux, opposé au modèle centralisé.

**IA (Intelligence Artificielle)**
Champ scientifique visant à créer des systèmes capables d'effectuer des tâches complexes. Dans le contexte du manifeste, c'est un "pharmakon" (remède et poison) et un dispositif politique qui transforme structurellement la propriété intellectuelle.

**Machine Learning (Apprentissage auto.)**
Sous-domaine de l'IA où les systèmes apprennent à partir de données (patterns) plutôt que par programmation explicite. C'est le paradigme qui rend possible la dissolution de l'attribution.

**Deep Learning (Apprentissage profond)**
Sous-domaine du *Machine Learning* utilisant des réseaux de neurones à multiples couches. Cette profondeur rend l'extraction de la provenance des données mathématiquement intraitable.

**Réseau de neurones artificiels**
Architecture computationnelle composée de couches de "neurones" interconnectés. C'est dans les poids de ces connexions que le savoir collectif se compresse.

**Paramètre / Poids**
Nombre flottant ajustable constituant la "mémoire" du réseau de neurones. Ils encodent des corrélations statistiques complexes, la matérialisation physique des communs statistiques.

**Transformer**
Architecture de réseau de neurones (2017) qui a rendu possibles les *LLM* modernes grâce au mécanisme d'attention, accélérant la compression et la recombinaison.

**LLM (Large Language Model)**
Modèle de langage de très grande taille, entraîné sur des milliards de mots et de code. Ils sont les agents principaux de la transmutation du code privé en communs statistiques.

**Token / Tokenisation**
Processus de fragmentation du texte ou du code en unités élémentaires ("tokens") que l'algorithme manipule mathématiquement. C'est la première étape de la dissolution de l'intégrité sémantique.

**Dataset / Corpus d'entraînement**
L'ensemble massif de données (Téraoctets de code) utilisées pour entraîner le modèle. L'article souligne que la prépondérance de sources *open source* (~60%) imprègne le modèle d'un "tropisme vers le partage".

**Entraînement (*Training*)**
Processus par lequel le modèle apprend en ajustant itérativement les milliards de poids pour minimiser l'erreur. L'article souligne le coût écologique massif de cette transformation du savoir.

**Espace Latent**
Espace mathématique abstrait où les données sont représentées sous forme compressée. Le manifeste le décrit comme un "océan probabiliste où toutes les solutions jamais écrites flottent en état de virtualité pure".

**Embedding**
Représentation vectorielle dense d'un élément dans l'espace latent. C'est à ce niveau que subsistent les "fantômes de licence" — traces spectrales, désormais mathématiquement inextricables.

**Inférence**
Phase où le modèle génère de nouvelles données (code) en réponse à un *prompt*. Ce processus probabiliste est la manière dont l'IA "fabrique du domaine public".

**Prompt Engineering**
L'art de formuler des instructions optimales pour obtenir des résultats d'un LLM. Le prompt devient la "nouvelle littératie" et l'"incantation" adressée à l'intelligence collective compressée.

**Statistiques**
Sciences de l'analyse et de l'interprétation de données (etym. "sciences de l'Etat"). L'article se réfère aux *communs statistiques* pour souligner que le savoir émerge par sédimentation et recombinaison probabiliste.
