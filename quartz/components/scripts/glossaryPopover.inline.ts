...
function createPopup(content: HTMLElement, title: string, url: string, sourceRect: DOMRect): HTMLElement {
  ...
  popup.style.left = `${Math.min(sourceRect.right + 10, window.innerWidth - 420)}px`
  popup.style.top = `${Math.min(sourceRect.top, window.innerHeight - 400)}px`
  ...
  if (popupRect.right > window.innerWidth - 10) {
    popup.style.left = `${sourceRect.left - popupRect.width - 10}px`
  }
...
async function showGlossaryPopup(link: HTMLAnchorElement) {
  ...
  try {
    ...
    const popup = createPopup(contentClone, title, link.href, sourceRect)
    activePopup = popup
...
// Close non-pinned popup when clicking outside
document.addEventListener("click", (e) => {
  if (activePopup &&
      !activePopup.classList.contains("pinned") &&
      !activePopup.contains(e.target as Node) &&
      !(e.target as HTMLElement).closest(".glossary-link")) {
    activePopup.remove()
    activePopup = null
  }
})
